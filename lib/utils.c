...
String file_to_str(char* filename) {
  if (access(filename, F_OK | R_OK) != 0) {
    return_err(utils_err, STR_EMPTY, "%s(): %s does not exist or not readable", __FUNCTION__, filename);
  }
  FILE* fp = fopen(filename, "r");
  if (fp == NULL) {
    fclose(fp);
    return_err(utils_err, STR_EMPTY, "%s(): failed to open %s", __FUNCTION__, filename);
  }
  ...
  while ((ch = fgetc(fp)) != EOF) {
    if (str_insert(&file, STR_END, ch) != OK) {
      fclose(fp);
      return_err(utils_err, STR_EMPTY, "%s(): string insertion error %s\n%s", __FUNCTION__, filename, str_err);
    }
  }
  fclose(fp);
...
int8_t str_to_file(char* filename, String content) {
  ...
  if (fp == NULL) {
    fclose(fp);
    return_err(utils_err, BAD, "%s(): failed to open %s", __FUNCTION__, filename);
  }
  size_t bytes_written = fwrite(content.str, sizeof(char), content.length, fp);
  if (bytes_written != content.length) {
    fclose(fp);
    return_err(utils_err, BAD, "%s(): couldn't finish writing to %s", __FUNCTION__, filename);
  }
  fclose(fp);
