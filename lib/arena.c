...
Arena *arena_init(uint64_t capacity) {
    if (capacity <= 0)
      return_err(arena_err, NULL, "%s(): Capacity of arena must be greater than 0.", __FUNCTION__);

    uint8_t *new_buffer = malloc(sizeof(uint8_t) * capacity);
    if (new_buffer == NULL)
      return_err(arena_err, NULL, "%s(): Failed to allocate memory for new buffer.", __FUNCTION__);

    Arena *arena = malloc(sizeof(Arena));
    if (arena == NULL)
      return_err(arena_err, NULL, "%s(): Failed to allocate memory for arena", __FUNCTION__);
...
void *arena_alloc(Arena *arena, uint64_t size) {
  if (size > arena->capacity)
    return_err(arena_err, NULL, "%s(): The requested size must be less than or equal to arena capacity", __FUNCTION__);
